...
The application strictly divides the user experience into two distinct modes. Understanding these two modes is crucial to understanding the frontend logic and how the backend manages the Raspberry Pis.

1. **Drawing Mode**: A live 3D environment where users create new art using hand tracking via a webcam (MediaPipe and Three.js). The UI overlay is hidden by default to prioritize the 3D canvas. While a client is in drawing mode, the backend runs a slideshow on the Raspberry Pis so they don't sit idle.
2. **Review Mode (Non-Drawing Mode)**: The default 2D interaction state. In review mode, users can view past on-chain drawings, convert SVG files to NAPLPS, mint drawings to the blockchain, navigate through history, or sit back and watch the slideshow. 
...
6. **The Slideshow Cycle**: One cycle of slides — chain tokens alternating with the slideshow's local files (`rpi-cycle.js`; see *The Pi while in drawing mode*) — that keeps the Pis busy while a page is in **drawing mode**, and is the slideshow a page runs in **review mode**. Pages only say when they join and leave, and a page running its slideshow gets each slide as the Pis do.
...
| `POST /api/naplps/simplify` | Simplifies a drawing to fit within `TEZOS_MAX_BYTES` — removes duplicate colors and degenerate polygons, applies `poly-simplify` at 0.8 quality, and if still too large, aggressively cascades through dropping small polygons, enforcing point budgets, and downgrading coordinate precision. |
...
The frontend is a single-page app (`index.html`) driven by the two core modes: **review mode** for viewing and managing NAPLPS files, and **drawing mode** for live 3D creation.

### Core Components

**`index.html`** - Main application entry point containing:
- p5.js canvas for 2D NAPLPS rendering (used in **review mode**). Its draw loop stops while **drawing mode** is up — the overlay covers it, and the reveal of whatever arrived as the page opened would otherwise run behind the overlay, taking frames and CPU from the drawing — and leaving **drawing mode** starts it again.
- Drag-and-drop loading for `.nap` or `.svg` files (used in **review mode**), which updates the yellow status text to confirm loading
...
The 3D **drawing mode** is an ES-module-based environment built on Three.js and MediaPipe:
- `drawing.js` - Main entry point. Sets up the Three.js scene and camera, takes the hand tracker's results, and runs the render loop.
- `hands.js` - `HandTracker` runs the gesture recognizer off the main thread and queues its results; `HandInput` decides which hand in each result is which (see *Hands, off the main thread*).
- `hands-worker.js` - One MediaPipe `GestureRecognizer` in a module worker. `HandTracker` runs two of them.
- `tools.js` - `Stroke` and `Frame` classes for managing 3D lines and points.
- `controller.js` - A hand as a set of buttons and a position, driven by recognizer results: One Euro filtering of the fingertip, debounced signs, and the times strokes start and end.
...
**Hands, off the main thread.** The recognizer is the most expensive thing drawing mode does — on a Raspberry Pi 4, ~380 ms a frame with a hand in view — and it used to run synchronously in the render loop, which held drawing mode to two or three frames a second whenever anyone used it. It now runs in workers (`hands-worker.js`), fed one frame at a time as an `ImageBitmap`, and the page renders at its own pace, taking results as they arrive. There are two recognizers, each in its own worker: one looking for up to two hands and one for a single hand, which skips the palm detection the two-hand one runs on every frame and so samples a single drawing hand about twice as often. `HandTracker` gives the single-hand one the frames while one hand is in view, and the two-hand one the frames when there is no hand, two hands, or a hand making a sign with a two-handed meaning (a fist or a thumb), with a look for a second hand once a second — but not mid-stroke, when that look would cost the stroke its samples. Both use MediaPipe's CPU delegate: its "GPU" delegate is WebGL, and on a Pi it shares the one GPU with the scene. Each is warmed up on a blank frame before it is used, and loaded once per page rather than on every entry to **drawing mode**. Where a module worker can't be had, the recognizer runs on the main thread as before.

Every result carries the time its camera frame was taken, and everything downstream runs on that clock rather than on animation frames, so drawing mode behaves the same at five results a second as at thirty. `HandInput` matches each result's hands to the controllers by where each controller's hand was last seen (the recognizer's own order means nothing), merges the recognizer's occasional duplicate of one hand, and declares a hand gone after two results running without it. Each `Controller` smooths the fingertip with One Euro filters — heavy smoothing at rest, little lag in motion — and debounces the recognizer's labels into signs: a sign has to be seen on two results (three when the hand is moving fast) before it counts, and "None" has to persist for longer before it can end a sign that was held. A stroke starts from where its sign was first seen, from the controller's recent path, and ends where the sign that ended it was first seen; the line tool is sticky through "None" and ends on any other sign or when the hand goes. Points are placed on the camera ray through the fingertip, so a stroke lands on screen exactly under the finger whatever the (noisy) depth. The pointer eases between results so it glides rather than jumps.

Held gestures are timed on what the camera saw: from the sign's first sighting to now, but never more than one result past its latest sighting, so a hand that drops out of view stops the clock. A one-handed hold that fills while another hand is making the same sign, not yet confirmed, waits for it (up to 1.5 s): that is a mint or a delete-all in the making, not a recentre or an undo.

**Render scale.** On a Pi the VHSC pass alone costs about a frame at 60 Hz at full screen, so the renderer can draw at a lower resolution and let the browser scale it up. When frames run long the scale drops a step (to 0.5 at most, which is still more pixels than the 640x480 the drawing is made in), and keeps it only if frames actually got faster — they can be slow for reasons resolution doesn't touch — climbing back a step at a time when there is room. A machine that keeps up stays at 1.
...
**From stroke to NAPLPS.** A stroke leaves **drawing mode** as a radically simplified structure: a single filled polygon representing the stroke's outline. `Stroke.toScreenPath()` projects the centreline into the flat 0–1 space the format draws in, measures the brush width there — across the view, so a stroke drawn toward the camera keeps its weight instead of collapsing to a hairline in its own plane — and provides the projected points and radii. These points are simplified using the Douglas-Peucker algorithm before being expanded into left and right edges that join to form a single continuous polygon. This avoids the complex corner fans, caps, and overlap logic used previously, producing rock-solid rendering that looks consistent on both the browser canvas and the Pi.
...
**The polyline is what gets projected.** Strokes are kept as the polyline the hand drew, and the 2D pieces are regenerated from its projected points, so nothing is flattened twice. The one thing that used to break that rule was the per-stroke z-offset that keeps the 3D preview from z-fighting: it was added to the points themselves, so it moved the *encoded* drawing too, by a distance that grew with every stroke added. `Stroke.offsetAlongNormal()` now only records it and `Frame._refreshGeometry()` puts it on the preview geometry, where a trick for the depth buffer belongs. Completed strokes all live in one vertex-coloured mesh (`StrokeBatch` in `tools.js`): a stroke is appended once, when it ends, and undo pops it, so the whole drawing is one draw call however many strokes it has — attract mode replays drawings of 600 to 1900 polygons, and a mesh each cost a Pi 10–30 ms of every frame in draw calls alone. The strokes being drawn keep a mesh each, rebuilt at most once a frame (`Frame.flush()`).

**Smooth strokes from sparse samples.** `Stroke.refine()` fits a centripetal Catmull-Rom spline through a finished stroke's samples, so a hand tracked four or five times a second still draws round curves, and the brush's taper runs by distance along the stroke rather than by point count, so it doesn't depend on how often the hand was sampled. Points closer together than a hundredth of a unit are one point (a stroke that never moves further is kept as a dot). Points carry the time they were measured, and `Frame.endStroke(id, cutTime)` drops the ones after the cut; a stroke ended without one — attract mode's replays, the mouse — keeps every point.
...
Two overlays stand outside that flag, both inside `#drawing-container`: the mint status line and the gesture card (see *The gesture card*). **Drawing mode** spends most of its time with the chrome away, and a kiosk that can say nothing about a mint in flight, or about the gestures it is waiting for, is one nobody can work — so those two keep their own timers and ignore "h".
