...
**The Pi while drawing.** With the overlay up, the page's canvas has nothing on it for the Pi to follow, so the Pi gets a cycle of its own. `index.html` starts it on entering drawing mode — and so as the page opens — and stops it on leaving, the Pi keeping whichever drawing it was sent last. Every `slideshow_interval` seconds one drawing goes out through `NapClient.sendToRpi()`, the Pi-only route the slideshow and the arrow keys use, so other clients' screens are left alone, and the turns alternate: a token from the chain, then one of the slideshow's local files, then the next token, the chain going first. The tokens keep to one order — the newest first, then each one before it down to #0, then round to the newest again. The local file is a random pick from the slideshow's list, read by the same `readNapFile()` the slideshow loads through, so the Pi gets the same bytes from a file either way, but it never goes onto the canvas, which is behind the overlay. A mint resets the count to the newest. The watcher sends a new token to the Pi the moment it finds it, and the page takes a chain message newer than any it has seen for exactly that, so the mint counts as the chain's turn: a full interval to itself, then a local file, then the token before it. That goes for anyone's mint, not only one made by this page's gesture, since the Pi is showing it either way. A turn passes whether or not it had anything to send, so a chain that can't be read, or a file that won't load, costs only its own slot. A read still out when the cycle is stopped or reset is dropped rather than sent — otherwise an old drawing could land on the Pi over the mint that just arrived. Ids with no drawing behind them are passed over within the chain's turn, a few reads at a time, and with no Pi configured (`rpiEnabled` in `GET /api/config`) the cycle reads nothing.
