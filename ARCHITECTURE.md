...
**The Pi while drawing.** With the overlay up, the page's canvas has nothing on it for the Pi to follow, so the Pi reads along the chain instead. `index.html` starts the cycle on entering drawing mode — and so as the page opens — and stops it on leaving, the Pi keeping whichever token it was sent last. Every `slideshow_interval` seconds one token goes out through `NapClient.sendToRpi()`, the Pi-only route the slideshow and the arrow keys use, so other clients' screens are left alone: the newest first, then each one before it down to #0, then round to the newest again. A mint resets the count to the newest. The watcher sends a new token to the Pi the moment it finds it, and the page takes a chain message newer than any it has seen for exactly that, so the new drawing gets a full interval to itself before the cycle carries on from the token before it. That goes for anyone's mint, not only one made by this page's gesture, since the Pi is showing it either way. A read still out when the cycle is stopped or reset is dropped rather than sent — otherwise an old token could land on the Pi over the mint that just arrived. Ids with no drawing behind them are passed over within the tick, a few reads at a time, and with no Pi configured (`rpiEnabled` in `GET /api/config`) the cycle reads nothing.
